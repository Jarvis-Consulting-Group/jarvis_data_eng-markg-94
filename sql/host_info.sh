...
cpu_model=$(echo "$lscpu_out"  | grep -E "^Model name:" | awk '{print $2}' | xargs)
cpu_mhz=$(echo "$lscpu_out"  | grep -E "^CPU MHz:" | awk '{print $2}' | xargs)
l2_cache=$(echo "$lscpu_out"  | grep -E "^L2 cache:" | awk '{print $2}' | xargs)
